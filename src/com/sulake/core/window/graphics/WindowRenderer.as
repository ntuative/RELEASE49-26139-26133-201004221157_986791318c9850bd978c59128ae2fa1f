package com.sulake.core.window.graphics
{
   ...
   public class WindowRenderer implements IWindowRenderer
   {
      ...
      public function render() : void
      {
         ...
         while(_loc4_ < _loc2_)
         {
            _loc1_ = var_603.pop() as WindowController;
            if(!_loc1_.disposed)
            {
               _loc3_ = getWindowRendererItem(_loc1_);
               var_310.x = _loc3_.dirty.x;
               var_310.y = _loc3_.dirty.y;
               var_310.width = _loc3_.dirty.width;
               var_310.height = _loc3_.dirty.height;
               renderWindowBranch(_loc1_,_loc3_,var_310);
            }
            _loc4_++;
         }
      }
      ...
      {
         param1.removeEventListener(WindowDisposeEvent.const_577,windowDisposedCallback);
         var _loc2_:WindowRendererItem = var_311[param1] as WindowRendererItem;
         if(_loc2_ != null)
         {
            _loc2_.dispose();
      ...
      private function renderWindowBranch(param1:WindowController, param2:WindowRendererItem, param3:Rectangle) : void
      {
         ...
         if(param1.visible)
         {
            if(getDrawLocationAndClipRegion(param1,param3,var_1538,var_1539))
            {
               if(param2.render(param1,var_1538,var_1539))
               {
      ...
      public function registerRenderable(param1:IWindow) : void
      {
         ...
         if(_loc2_ == null)
         {
            _loc2_ = new WindowRendererItem(this,_drawBufferAllocator,var_744);
            var_311[param1] = _loc2_;
            param1.addEventListener(WindowDisposeEvent.const_577,windowDisposedCallback);
         }
      }
      ...
      public function addToRenderQueue(param1:IWindow, param2:Rectangle, param3:uint) : void
      {
         ...
         if(getWindowRendererItem(param1).invalidate(param1,param2,param3))
         {
            ...
            var_310.x = param2.x - param1.x;
            var_310.y = param2.y - param1.y;
            var_310.width = param2.width;
            var_310.height = param2.height;
            if(param1.testParamFlag(WindowParam.const_35))
            {
               while(param1.testParamFlag(WindowParam.const_35))
               {
                  ...
                  var_310.offset(param1.x,param1.y);
                  param1 = _loc4_;
               }
            }
            getWindowRendererItem(param1).invalidate(param1,var_310,WindowRedrawFlag.const_258);
            if(var_603.indexOf(param1) == -1)
            {
               var_603.push(param1);
            }
         }
