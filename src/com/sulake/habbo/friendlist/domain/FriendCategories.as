package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         var _loc5_:* = null;
         flushReceivedStatus();
         findCategory(FriendCategory.const_379).received = true;
         findCategory(FriendCategory.const_806).received = true;
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc2_.updatedFriends)
         {
            ...
            var_427.messenger.setFollowingAllowed(_loc4_.id,_loc4_.followingAllowed && _loc4_.online);
            _loc6_ = isFriendOnline(_loc4_.id);
            if(_loc6_ && !_loc4_.online)
            {
               var_427.messenger.setOnlineStatus(_loc4_.id,_loc4_.online);
               var_427.notifications.addOfflineNotification(_loc4_.name,_loc4_.realName);
            }
            if(!_loc6_ && _loc4_.online)
            {
               var_427.messenger.setOnlineStatus(_loc4_.id,_loc4_.online);
               var_427.view.setNewMessageArrived();
               var_427.notifications.addOnlineNotification(_loc4_.name,_loc4_.figure,_loc4_.realName);
            }
